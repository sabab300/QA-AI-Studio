...
from UI.KnowledgeHub.discovery_step_confirmation_dialog import (
    DiscoveryStepConfirmationDialog,
)
...
class SmartUploadPage(QWidget):
    ...
    def __init__(self):
        ...
        # Guided business-flow capture state (AI Smart Upload -> URL
        # Knowledge). flow_context holds the operator-confirmed
        # Application/Business Process/Variant names for the whole
        # flow; captured_steps accumulates one confirmed step dict
        # per screen until the operator marks a step as final.
        self.flow_context = None
        self.captured_steps = []
        self.flow_saved = False
    ...
    def build_ui(self):
        ...
        # ==================================================
        # Guided Business Flow Capture (URL Knowledge)
        #
        # Shown once an authenticated Playwright session is ready.
        # The operator drives the real business process by hand in
        # the still-open browser window and clicks "Capture This
        # Screen" after each step; every capture is reviewed in
        # DiscoveryStepConfirmationDialog before it is ever saved.
        # ==================================================

        self.flow_group = QGroupBox(
            "Guided Business Flow Capture"
        )

        flow_layout = QVBoxLayout(self.flow_group)

        flow_layout.setContentsMargins(15, 20, 15, 15)

        flow_layout.setSpacing(10)

        self.flow_status_label = QLabel(
            "Authenticate via “Add URL” above to start a guided capture."
        )
        ...
        self.capture_step_btn = QPushButton("Capture This Screen")

        self.capture_step_btn.setStyleSheet(
        ...
        flow_buttons.addStretch()

        flow_layout.addLayout(flow_buttons)

        self.captured_steps_list = QListWidget()

        self.captured_steps_list.setMinimumHeight(90)

        self.captured_steps_list.setSizePolicy(
            QSizePolicy.Expanding,
            QSizePolicy.Fixed
        )

        flow_layout.addWidget(self.captured_steps_list)
    ...
    def authenticate_url_with_credentials(
        ...
        # NOTE: this thread is NOT a short-lived "run once and finish"
        # worker any more. Once authenticated it idles on its own
        # command queue (see SmartUploadWorker) so that "Capture This
        # Screen" can be clicked as many times as the real business
        # flow needs, all from this one Playwright-safe thread.
        self.url_discovery_worker.moveToThread(self.url_discovery_thread)

        # Reset guided-flow accumulation state for this run.
        self.flow_context = None
        self.captured_steps = []
        self.flow_saved = False
        ...
        self.url_discovery_worker.flow_ready_signal.connect(
            self.on_flow_ready
        )
    ...
    def start_authenticated_url_discovery(
        ...
        # NOTE: start_authenticated_url_discovery() / this whole
        # two-thread (separate auth worker + separate URLDiscoveryWorker)
        # path is not wired to any button and is not reachable from the
        # live UI (on_url_analysis_finished() uses
        # authenticate_url_with_credentials()'s single-thread
        # SmartUploadWorker instead, which is required for Playwright
        # thread-safety — see smart_upload_worker.py's module
        # docstring). Left in place rather than deleted to keep this
        # change focused, but do not wire a new button to this method
        # without first switching it to the same single-thread pattern.
    ...
    def on_flow_ready(self, payload):
        """
        Fired once SmartUploadWorker has authenticated and is idling
        on its command queue, ready for "Capture This Screen" clicks.

        This is where the operator confirms the hierarchy (Application
        / Business Process / Variant) the whole flow will be filed
        under, before a single element is captured.
        """

        if not isinstance(payload, dict) or not payload.get("success"):
            self.log.append(
                "Authenticated session did not report ready state; "
                "guided capture cannot start."
            )
            self._restore_url_controls()
            return

        source_url = (
            self.url_discovery_source_url
            or payload.get("url")
            or self.selected_url
            or ""
        )

        dialog = BusinessFlowStartDialog(source_url, parent=self)

        if dialog.exec() != QDialog.Accepted:

            self.log.append(
                "Guided capture cancelled before it started."
            )
        ...
        self.flow_context = dialog.values()
        self.captured_steps = []
        self.flow_saved = False

        self.captured_steps_list.clear()

        self.flow_group.setVisible(True)

        self.flow_status_label.setText(
            "Drive the business process in the browser window, then "
            "click “Capture This Screen” after each step. Click "
            "“Cancel Flow” if you want to stop without saving."
        )
        ...
        self.log.append(
            "Ready for guided capture. Application="
            f"{self.flow_context.get('application_name')}, "
            f"Business Process={self.flow_context.get('business_process_name')}, "
            f"Variant={self.flow_context.get('variant_name')}."
        )

    def _on_capture_screen_clicked(self):

        if self.url_discovery_worker is None:
            QMessageBox.warning(
                self,
                "Guided Capture",
                "There is no active authenticated session to capture from.",
            )
            return

        self.capture_step_btn.setEnabled(False)

        step_label = f"Step {len(self.captured_steps) + 1}"

        self.log.append(f"Requesting capture of the current screen ({step_label})...")

        self.url_discovery_worker.request_capture(step_label=step_label)

    def on_step_ready(self, scan):
        """
        Fired when the worker finishes scanning whatever screen the
        operator manually navigated to. Nothing from `scan` reaches
        the database until the operator reviews and confirms it here.
        """

        self.capture_step_btn.setEnabled(True)

        if not isinstance(scan, dict) or not scan.get("success"):
            error = scan.get("error") if isinstance(scan, dict) else "Unknown error."
            self.log.append(f"Capture failed: {error}")
            QMessageBox.warning(
                self,
                "Capture Failed",
                f"Could not read this screen:\n{error}\n\n"
                "You can try “Capture This Screen” again once the page "
                "has finished loading.",
            )
            return

        step_index = len(self.captured_steps) + 1

        dialog = DiscoveryStepConfirmationDialog(
            scan,
            step_index=step_index,
            default_page_name=scan.get("page_title") or "",
            parent=self,
        )

        if dialog.exec() != QDialog.Accepted:
            self.log.append(f"Step {step_index} discarded — capture this screen again if needed.")
            return
        ...
        self.captured_steps_list.addItem(
            f"Step {step_index}: {confirmed.get('step_name')} — {included} element(s)"
            + (" [final step]" if confirmed.get("is_end_step") else "")
        )

        self.log.append(
            f"Step {step_index} confirmed and queued: {included} element(s) on "
            f"'{confirmed.get('page_name')}' / '{confirmed.get('tab_name')}'."
        )

        if confirmed.get("is_end_step"):
            self._finish_flow_and_save()

    def _save_captured_steps_to_repository(self):
        """
        Persists everything captured so far via
        DiscoveryRepository.save_flow_result(). Shared by the normal
        "operator marked the final step" path and by the
        unexpected-disconnect recovery path in on_flow_worker_finished,
        so a browser crash mid-flow doesn't have to mean losing steps
        the operator already reviewed and confirmed.
        """
        ...
        flow_context = self.flow_context or {}

        source_url = (
            self.url_discovery_source_url
            or self.selected_url
            or self.captured_steps[0].get("page_url")
            or ""
        )
        ...
        try:
            ...
            save_result = repository.save_flow_result(
                self.captured_steps,
                source_url=source_url,
                auth_type=self.url_discovery_auth_type,
            ...
            self.log.append(f"Saving the captured business flow failed: {ex}")
            QMessageBox.warning(
                self,
        ...
        self.log.append(
            ...
            f"{save_result.get('application_id')}, Business Process ID "
            f"{save_result.get('business_process_id')}, Variant ID "
            f"{save_result.get('variant_id')}."
        )
    ...
    def _on_cancel_flow_clicked(self):
        ...
        if self.captured_steps:
            confirm = QMessageBox.question(
                self,
                "Cancel Flow",
                f"{len(self.captured_steps)} confirmed step(s) have not been saved yet. "
                "Discard them and close the browser?",
            )
        ...
        self.captured_steps = []
        self.flow_context = None
        self.flow_saved = True  # nothing left to save on worker teardown

        if self.url_discovery_worker is not None:
            self.url_discovery_worker.request_finish(keep_browser_open=False)

    def on_flow_worker_finished(self, result):
        """
        Fired once the worker thread's command loop exits — either
        because the operator finished/cancelled the flow, or because
        authentication failed, or because something unexpected went
        wrong mid-flow (e.g. the operator closed the browser window
        directly). In the last case, anything already confirmed by
        the operator is saved here rather than silently lost.
        """

        success = isinstance(result, dict) and result.get("success")

        if not success and self.captured_steps and not self.flow_saved:

            error = result.get("error", "The session ended unexpectedly.") if isinstance(result, dict) else "The session ended unexpectedly."

            recover = QMessageBox.question(
                self,
                "Session Ended Unexpectedly",
                f"{error}\n\n"
                f"{len(self.captured_steps)} confirmed step(s) were captured before this "
                "happened. Save them to Knowledge Hub now?",
            )
            ...
            self.log.append(f"Guided capture session ended: {error}")
